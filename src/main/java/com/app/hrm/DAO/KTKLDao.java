 ...
 * @author ADMIN
 */
public class KTKLDao extends HRMDAO<KTKL,String>{
    final String INSERT_SQL = "INSERT INTO Khen_KyLuat(MaKTKL,NoiDung,Ngay,MaNV,Loai,tien) values(?,?,?,?,?,?)";
    final String UPDATE_SQL = "UPDATE Khen_KyLuat SET NoiDung = ?,Ngay= ?,Loai=?,tien=?,manv=?  WHERE maktkl = ?";
    final String DELETE_SQL = "DELETE FROM Khen_KyLuat WHERE maktkl = ?" ;
    final String SELECT_ALL_SQL = "SELECT * FROM Khen_KyLuat";
    final String SELECT_BY_ID_SQL = "SELECT * FROM Khen_KyLuat where maktkl = ?";
    @Override
    ...
    public void update(KTKL e) {
        JdbcHelp.update(UPDATE_SQL, e.getNoidung(),e.getNgay(),e.isLoai(),e.getTien(),e.getMaNV(),e.getSoKTKL());
    }
    ...
    @Override
    public KTKL selectById(String id) {
